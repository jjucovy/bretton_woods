...
async function loadUsersFromDB(retries = 3) {
    if (Object.keys(globalState.users).length === 0) {
        ...
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                // Use dbSync for consistency with error handling
                const users = await dbSync(db.getAllUsers);
                if (!users) throw new Error('Failed to retrieve users from DB.');

                users.forEach(user => {
                    globalState.users[user.username] = {
                        password: user.password_hash,
                        playerId: `player_db_${user.user_id}`,
                        userId: user.user_id, // Store userId for easy lookups
                ...
                return; // Successfully loaded, exit loop and function
            } catch (err) {
                console.warn(`⚠️ MySQL connection attempt ${attempt}/${retries} failed: ${err.message}`);
                if (attempt < retries) {
                    // Correct syntax for async delay
...
app.get('/api/export-state/:roomId', (req, res) => {
    ...
    const exportData = {
        exportedAt: new Date().toISOString(), // Completed the date method
        roomState: room // Assuming the intent was to export the full room state
    };
    res.json(exportData); // Added the response send
});
